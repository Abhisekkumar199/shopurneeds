        ...
        <div class="page-hero page-container" id="page-hero">
            <div class="padding d-flex">
                <div class="page-title">
                    <h2 class="text-md text-highlight"> ייבא מוצר</h2>
                </div>
                <div class="flex"></div>
                <div><a href="product-list" class="btn btn-md text-muted"><span class="d-none d-sm-inline mx-1"> הצג את כל המוצרים</span> <i data-feather="arrow-right"></i></a></div>
            </div>
        </div>
                        ...
                        <div class="card"> 
                            <div class="card-body"> 
                                ...
                                <div class="form-group row">
                                    <label class="col-sm-3 col-form-label">העלה קובץ CSV</label>
                                    <div class="col-sm-9">
                                        <input name="uploaded" type="file" size=20 class="form-control col-md-3" required />  
                                    </div>
                                </div>
                                
                                <button type="submit" name="upfile" class="category btn w-sm mb-1 btn-success" style="float: right; margin-right: 12px;">לשמור</button>
                            </div> 
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card"> 
                            <div class="card-body"> 
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-9">
                                        <?php 
                                        $sqlasdas=mysqli_query($conn,"select * from shopurneeds_product_dummy where uploaderror !='0' and insertid='".$_REQUEST['insertid']."'");
                                        $numsdsd=mysqli_num_rows($sqlasdas);
                                        if($numsdsd==0)
                                        {  
                                        ?>
                                        <p>המוצר הועלה בהצלחה, אנא לחץ כאן להשלמת תהליך ההעלאה		</p>
                                        <button type="submit" class="category btn w-sm mb-1 btn-success" style="float: right; margin-right: 12px;">לשמור</button>
                                            <?php } else { ?>
                                            <?php while($rowss=mysqli_fetch_array($sqlasdas)) { ?>
												<p> <?php echo  $rowss ['sku'];?>-<?php echo  $rowss ['product_title'];?>מכיל שגיאה, אנא בדוק. <?php if($rowss['uploaderror']=="1") { echo "mandatory field missing"; }?><?php if($rowss['uploaderror']=="2") { echo "SKU already exist."; }?></p>
										    <?php } ?>
                                        <?php } ?>
                                    </div>
                                </div>
