...
if(!empty($userid)){
		...
		$user2 = $db->getEnquiryByUsersCurrent($userid,$tabName);
		if ($user2 != false) {
...
$EnquiryResult[] = array("enquiryid"=>$proenquiryrows['id'],"companyname"=>$companyName,"userid"=>$proenquiryrows['user_id'],"sizeFrom"=>$proenquiryrows["sizefrom"], "sizeTo"=>$proenquiryrows["sizeto"],  "sizeTo2"=>$proenquiryrows["sizeto2"], "sizeTitle1"=>ucwords(strtolower($catrows2['sizetitle1'])), "sizeTitle2"=>ucwords(strtolower($catrows2['sizetitle2'])), "sizeOption"=>$catrows2['check_size_option'], "price"=>$proenquiryrows['pirce'],"maxPrice"=>$proenquiryrows['maxprice'],"description"=>$proenquiryrows['message'],"catid"=>$proenquiryrows['categoryid'],"categoryname"=>$catrows2['categoryname'],"imageurl"=>'https://localhost/project/shopurneeds/uploads/categoryimages/'.$catrows2['uploadimage1'],"date"=>$proenquiryrows['adddate']);
				$EnquiryResult2='';
				}
				$hours=''; } }
...
echo json_encode($response);
			...
			$response["header"] = "Current Enquiry";
			echo json_encode($response);
		}
