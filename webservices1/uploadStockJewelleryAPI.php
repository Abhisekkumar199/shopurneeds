...
if(!empty($userId)){
			...
			$user = $db->uploadStockJewelleryByUserId($userId,$certificate,$diamondColor,$diamondClarity,$metal,$face,$totalValue,$grossWeight,$ear,$neck,$chest,$wrist,$finger,$diamondRate,$uploadImage);

			if ($user) {
			$response["status"] = "200";
			$response["msg"] = "Jewellery Stock Successfully Uploaded.";
				echo json_encode($response);
			} 
			else {
			$response["status"] = "400";
				$response["msg"] = "Error occurred in Jewellery upload stock!";
				echo json_encode($response);
			}
}
else
