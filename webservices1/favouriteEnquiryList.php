...
if(!empty($userId)){
		$user2 = $db->getFavouriteEnquiryList($userId);
		if ($user2 != false) {
		while($proenquiryrows2 = mysql_fetch_array($user2)) { 
			$response["message"] = "Success";
			$response["header"] = "My Favorite";
			
			$proenquiryrows = mysql_fetch_array(mysql_query("SELECT * FROM jewellersmandi_product_enquiry where displayflag='1' and id='".$proenquiryrows2['product_id']."'   ORDER BY id DESC"));
