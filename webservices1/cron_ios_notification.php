...
$db = new DB_Functions();
$notiSql = mysql_query("select * from jewellersmandi_notification_message where ios_status='0' order by id asc limit 1");
if(mysql_num_rows($notiSql)>0) {
$notiRows = mysql_fetch_assoc($notiSql);
$notiSql = mysql_query("update jewellersmandi_notification_message set ios_status='1' where id='".$notiRows['id']."'");
...
$messiostitle=$notiRows['title'];
$messioskeymsg=$notiRows['message'];
...
if (!$result) 
	...
	echo 'Message successfully delivered' . PHP_EOL. $customerRows['ios_count'];
