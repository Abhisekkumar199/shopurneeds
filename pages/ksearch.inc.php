...
{ 
    $ksearch = trim($_REQUEST['ksearch']); 
    $ksearch= isset($_GET['ksearch']) ? " and (searchkeyword like '%$ksearch%' or productname like '%$ksearch%')" : "";
     
    $prodcat_query  = mysqli_query($conn,"select distinct id from ".$sufix."product where displayflag='1' {$ksearch}");
    while($sql_prod = mysqli_fetch_assoc($prodcat_query)) 
    {
    	$prod_ids .= $sql_prod['id'].',';
    } 
    $prod_ids = substr($prod_ids,0,-1); 
    $range123 ="and id in($prod_ids)";
    $rangesize123 ="and product_id in($prod_ids)"; 
                  ...
                  <div class="list-group">
                        ...
                        ?> 
                            <div class="list-group-item mb-10">
                                ...
                                ?>    
                                    <div class="checkbox">
                                      <label> <input id="size<?php echo $i; ?>" value="<?php echo $sizerosa['product_size']; ?>" name="size[]"  <?php if(in_array($sizerosa['product_size'],$_REQUEST['size'])) { echo "checked='checked'"; } ?> type="checkbox" class="sort_rang"> <?php echo $sizerosa['product_size']; ?></label>
                                    </div>
                                <?php  $i++; } ?>      
                                </div>
                            </div> 
