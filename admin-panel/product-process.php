...
$atr_val_id=$_REQUEST['atr_val_id']; 
	...
	{ 
	    mysqli_query($conn,"insert into ".$sufix."product_size (`product_id`,`product_size`,`product_mrp`,discount_percent,discount_value,`product_sellingprice`,`adddate`,qty,min_stock_qty) values ('".$id."','Free Size','".$costprice."','".$discount_percent."','".$discount_value."','".$sellingprice."',NOW(),'".$in_stock_qty."','".$_POST['min_stock_qty'][$i]."')");
...
?>	
    ...
    <script>window.location.href='https://localhost/project/shopurneeds/admin-panel/add-product-variant.php?id=<?php echo $id; ?>';</script> 
    <?php } else { ?>
