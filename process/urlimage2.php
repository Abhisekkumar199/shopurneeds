...
include("sql.php");
$salpr=mysqli_query($conn,"select * from shopurneeds_product where image2!='' order by id desc limit 1");
while($rowss=mysqli_fetch_array($salpr))
{
    $sqljjj=mysqli_query($conn,"update shopurneeds_product set displayflag='1',image2='' where id='".$rowss['id']."'");
    $fname=str_replace(" ","%20","https://localhost/project/shopurneeds/downloadimages/".$rowss['image2']);
    ...
    $sqljjj=mysqli_query($conn,"insert into shopurneeds_imageupload(pid,productimage,adddate,mainimage,sortid) values ('".$rowss['id']."','".$name12."',NOW(),'0','2')");
...
window.location = "https://localhost/project/shopurneeds/urlimage2.php"
